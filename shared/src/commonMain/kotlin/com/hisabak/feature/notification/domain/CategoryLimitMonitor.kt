 ...
 * most once per category per month (tracked in [alertStore]); we never re-alert when spend dips.
 */
class CategoryLimitMonitor(
    ...
    private val alertStore: CategoryLimitAlertStore,
     ...
     * Idempotent: the per-month [alertStore] dedup means re-evaluating never double-alerts.
     */
    ...
    private suspend fun evaluate(inputs: Inputs) {
        ...
        for (category in inputs.categories) {
            ...
            if (level == 0) continue
            val lastLevel = alertStore.lastLevel(category.id.value, periodMonth) ?: 0
            if (level <= lastLevel) continue

            fire(category, spent, limit, level)
            alertStore.record(category.id.value, periodMonth, level)
        }
    }
    ...
    private fun format(amountMinor: Long): String = formatNotificationAmount(currency, amountMinor)
