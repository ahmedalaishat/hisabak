...
class CompactAmountTest {
    ...
    fun `thousands abbreviate to K with two decimals`() {
        assertEquals("1.00K", compactAmount(1_000.0, arabic = false))
        assertEquals("4.80K", compactAmount(4_800.0, arabic = false))
    ...
    fun `minor-unit helper divides by 100 first`() {
        assertEquals("1.25K", compactAmountMinor(125_000, arabic = false))
    ...
    fun `rounds half up on the shortest decimal representation`() {
        ...
        assertEquals("12.35K", compactAmount(12_345.0, arabic = false))
    ...
    fun `arabic uses arabic-indic digits separators and suffixes`() {
        ...
        assertEquals("٤٫٨٠ أ", compactAmount(4_800.0, arabic = true))
