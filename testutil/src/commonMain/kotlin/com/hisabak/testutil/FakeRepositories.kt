    ...
    override fun observe(filter: TransactionFilter): Flow<List<Transaction>> = items.map { list ->
        ...
        list.filter { tx ->
            (filter.brandId == null || tx.brandId == filter.brandId) &&
                (search == null || tx.note?.contains(search, ignoreCase = true) == true) &&
                (dateFrom == null || tx.occurredAt >= dateFrom) &&
                (dateTo == null || tx.occurredAt < dateTo)
        }
    }
...
class FakeCategoryLimitAlertStore : CategoryLimitAlertStore {
    ...
    override suspend fun record(categoryId: String, periodMonth: Int, level: Int) {
        levels[categoryId to periodMonth] = level
    }
}
