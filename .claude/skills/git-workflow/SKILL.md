...
4. **Land the bump on `develop` via PR.** Both `main` and `develop` are protected — no
   ...
   #   scripts/changelog-to-whatsnew.sh: each bullet contributes its bold lead phrase, so write that lead as the storefront line).
   #   Bullets are taken whole: one over 500 chars is skipped entirely, and Hisabak's entries
   #   are long enough that usually only the FIRST fits. Order the section by significance and
   #   check the generated note (run the script) before merging.
