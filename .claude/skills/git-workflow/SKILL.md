...
4. **Land the bump on `develop` via PR.** Both `main` and `develop` are protected — no
   ...
   #   scripts/changelog-to-whatsnew.sh, 500-char cap). Each bullet contributes its bold lead
   #   phrase — or its first sentence — so write that lead as the storefront line; about ten
   #   fit. Order the section by significance and check the generated note (run the script)
   #   before merging.
